...
*turnstile_find_periods(PyObject *self, PyObject *args)
{
    ...
    double min_period, max_period, dperiod;

    if (!PyArg_ParseTuple(args, "OOOddd", &time_obj, &flux_obj, &ivar_obj,
    ...
    for (i = 0; i < nperiods; ++i) {
        ...
        chi2[i] = compute_chi2(data, period, epochs[i], tau);
    }
    ...
    PyObject *ret = Py_BuildValue("OOOO", period_array, depth_array,
                                  epoch_array, chi2_array);
    return ret;
}

static PyObject
*turnstile_compute_chi2(PyObject *self, PyObject *args)
{
    PyObject *time_obj, *flux_obj, *ivar_obj;
    double period, epoch;

    if (!PyArg_ParseTuple(args, "OOOdd", &time_obj, &flux_obj, &ivar_obj,
                          &period, &epoch))
        return NULL;
    ...
    // Compute the fit.
    double tau = get_duration(period),
           chi2 = compute_chi2(data, period, epoch, tau);
...
static PyMethodDef turnstile_methods[] = {
    ...
    {"compute_chi2",
     (PyCFunction)turnstile_compute_chi2,
     METH_VARARGS,
     "Compute the chi^2 of a period/phase."},
    {NULL, NULL, 0, NULL}
};
